...
Often it can be helpful to write text on a frame, whether that is to label bounding rectangles or to include a colour key. OpenCV provides us with a function similar to drawing shapes, which allows us to write text called `cv.putText()`.
...
- See this [link](https://docs.opencv.org/4.x/d6/d6e/group__imgproc__draw.html#ga0f9314ea6e35f99bb23f29567fc16e11) for available fonts. Pass _cv.FONT\_OF\_YOUR\_CHOICE_ to access the font. 
...
```python
def mouse_event_name(event, x, y, flags,*params):
  # Handle mouse event
```
...
`cv.setMouseCallback()` takes a reference to your mouse event function as an argument, which it calls itself, passing in the arguments seen in the above function prototype. Before we call `cv.setMouseCallback()` we must name the frame we are attaching the mouse event function to, using `cv.namedWindow()`. Please inspect the code below to see this in action.
...
### `cv.blur(image: cv.Mat, kernel_size: tuple) -> cv.Mat`
...
### `cv.GaussianBlur(image: cv.Mat, kernel_size: tuple, x_s, y_s) -> cv.Mat`
...
- x_s and y_s stand for the standard deviation in the x and y direction respectively. If y_s is not set or is zero, it will be the same value as x_s. If both x_s and y_s are set to zero, then the standard deviation is calculated from the kernel size automatically.
- Returns the blurred image.

### `cv.medianBlur(image: cv.Mat, kernel_size: int) -> cv.Mat`
...
### `cv.bilateralFilter(image: cv.Mat, d: int, colour_s: int, space_s) -> cv.Mat`
...
- colour_s is the standard deviation of colour intensities, while space_s is the standard deviation for how far a pixel is from the centre pixel. The greater space_s is, the more of an effect pixels further away have.
...
3. Create a subplot with dimensions (3 x 2) using matplotlib, recalling the function `plt.imshow()`. Display the original along side the blurred images making sure to use the function `plt.tight\_layout()` before using `plt.show()`. Remove the x and y ticks or each subplot calling `plt.xticks([])` and `plt.yticks([])`.
...
1. Choose one of the blurs from Task 4 and apply it to video frames. Don't use matplotlib for this task as it will be too slow, use `cv2.imshow()` instead.
